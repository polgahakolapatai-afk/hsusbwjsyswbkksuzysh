...
const number = conn.user.id.split(':')[0].replace(/@s\.whatsapp\.net$/, '');
    ...
    const baseButtons = [
      ...
      { buttonId: prefix + 'ping', buttonText: { displayText: 'KAVI SPEED' }, type: 1 },
...
I am alive now 🎈\n❤️‍🔥This is\`KAVI-MD\` — multi device whatsapp bot. . ! ❤️‍🔥
*┌────────────────────┐*
*├ \`⏰ 𝐔𝐩𝐭𝐢𝐦𝐞\`* :- ${rtime}
*├ \`🚨 𝐇𝐨𝐬𝐭\`* :- ${hostname}
...
*😎 THANK YOU USING FOR KAVI-MD WA BOT. . !*`,
...
*I'm KAVI-MD Multi Device Whatsapp Bot 🌟*
*┌────────────────────┐*
*├ \`⏰ 𝐔𝐩𝐭𝐢𝐦𝐞\`* : ${rtime}
*├ \`🚨 𝐇𝐨𝐬𝐭\`* : ${hostname}
*├ \`🎡 𝐏𝐫𝐞𝐟𝐢𝐱\`* : ${config.PREFIX}
...
*❤️‍🔥 you can easily use this super whatsapp bot ❤️‍🔥*`;
...
async (conn, mek, m, { reply, isOwner, isSachintha, isSavi, isSadas, isMani, isMe }) => {
  ...
  try {
    ...
    await reply(`♻️ *kavi-md restarting...*  
...
\`🛸 𝐏𝐋𝐀𝐓𝐅𝐎𝐑𝐌 :-\`     *${hostname}*\n
\`👾 𝐕𝐄𝐑𝐒𝐈𝐎𝐍 :-\`      *4.0.0*\n
\`❤️‍🔥 𝐎𝐖𝐍𝐄𝐑 :-\`      *Kavidu Rasanga*\n

*𝙆𝘼𝙑𝙄-𝙈𝘿 𝙄𝙎 𝙍𝙐𝙉𝙉𝙄𝙉𝙂 𝙎𝙈𝙊𝙊𝙏𝙃𝙇𝙔 💪*
`;
